          ...
          {user ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {hasRole("driver") && <DropdownMenuItem onClick={() => navigate({ to: "/driver" })}>Driver panel</DropdownMenuItem>}
                {hasRole("vendor") && <DropdownMenuItem onClick={() => navigate({ to: "/vendor" })}>Vendor panel</DropdownMenuItem>}
                {hasRole("admin") && <DropdownMenuItem onClick={() => navigate({ to: "/admin" })}>Admin panel</DropdownMenuItem>}
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={async () => {
                    await signOut();
          ...
          <Drawer open={open} onOpenChange={setOpen}>
            ...
            <DrawerContent>
              ...
              <div className="p-4 space-y-3">
                <Link to="/" className="block px-4 py-3 text-base font-medium rounded-lg hover:bg-white/10 transition-colors" onClick={() => setOpen(false)}>Home</Link>
                <Link to="/about" className="block px-4 py-3 text-base font-medium rounded-lg hover:bg-white/10 transition-colors" onClick={() => setOpen(false)}>About</Link>
                <Link to="/contact" className="block px-4 py-3 text-base font-medium rounded-lg hover:bg-white/10 transition-colors" onClick={() => setOpen(false)}>Contact</Link>
                {user && (
                  <>
                    <Link to="/dashboard" className="block px-4 py-3 text-base font-medium rounded-lg hover:bg-white/10 transition-colors" onClick={() => setOpen(false)}>Bookings</Link>
                    <Link to="/wallet" className="block px-4 py-3 text-base font-medium rounded-lg hover:bg-white/10 transition-colors" onClick={() => setOpen(false)}>Wallet</Link>
                  </>
                )}
                ...
                <div className="pt-4 border-t border-white/10">
                  {user ? (
                    <div className="space-y-3">
                      ...
                      <Button size="sm" variant="outline" className="w-full border-white/20 hover:bg-white/10" onClick={async () => { await signOut(); navigate({ to: "/" }); setOpen(false); }}>Sign out</Button>
                    </div>
                  ) : (
                    <Button size="sm" className="w-full premium-btn" onClick={() => { navigate({ to: "/auth" }); setOpen(false); }}>Sign in</Button>
                  )}
                </div>
              </div>
            </DrawerContent>

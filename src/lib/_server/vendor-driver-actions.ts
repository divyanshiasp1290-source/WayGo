// NOTE: This module is intentionally placed under src/lib/_server so it is importable from client-safe
// TanStack Start serverFn bridges without triggering "/server/**" import protection.
...
export const updateVendorDriver = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const patch = Object.fromEntries(
      ...
      aadhaar_upload_url: string;
      aadhaar_verification_status?: "verified";
      pan_number?: string;
      pan_upload_url?: string;
    ...
    const patch = {
      ...
      ...(data.status === "active"
        ? {
            ...
            aadhaar_verification_status: "verified" as const,
            pan_verification_status: "verified" as const,
          }
        : {}),
    };
...
export const verifyDriverDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // Vendor (not admin) verifies documents.
    ...
    await supabaseAdmin
      .from("drivers")
      .update({ [column]: data.status } as any)
      .eq("id", data.driverId);
    ...
    if (refreshed) {
      ...
      if (allVerified) {
        await supabaseAdmin
          .from("drivers")
          .update({ status: "active", verified: true, suspended_at: null })
          .eq("id", data.driverId);
      } else {
        await supabaseAdmin
          .from("drivers")
          .update({ status: "pending_verification", verified: false })
          .eq("id", data.driverId);
      }
    }
